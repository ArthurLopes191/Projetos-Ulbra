...
function getJumpedPiece(cells, pieces, from, to) {
    ...
    if (abs(distance.x) == abs(distance.y)) {
        var stepX = distance.x / abs(distance.x);
        var stepY = distance.y / abs(distance.y);
        var jumpedPiece = null;

        for (var i = 1; i < abs(distance.x); i++) {
            var jumpRow = from.row + (i * stepY);
            var jumpCol = from.col + (i * stepX);
...
function isMoveLegal(cells, pieces, piece, from, to) {
    ...
    // King
    if (Math.abs(piece.state) === 1.1) {
        var stepX = distance.x / abs(distance.x);
        var stepY = distance.y / abs(distance.y);
        var encounteredPiece = false;
        var encounteredState = null;
        var piecesEncountered = 0;

        for (var i = 1; i < abs(distance.x); i++) {
            var cellIndex = getCellIndex(from.row + (i * stepY), from.col + (i * stepX));
            if (cells[cellIndex].state != empty) {
                if (encounteredPiece) {
                    return false;
                }
                encounteredPiece = true;
                encounteredState = cells[cellIndex].state;
                if (integ(encounteredState) === integ(piece.state)) {
                    return false;
                }
                piecesEncountered++;
            }
        ...
        if (encounteredPiece && encounteredState != null && integ(encounteredState) !== integ(piece.state) && piecesEncountered == 1) {
            var cellIndex = getCellIndex(to.row, to.col);
            if (cells[cellIndex].state != empty) {
                return false;
            }
            return true;
...
function drawBoard(origin, cellWidth, boardCanvas) {
	...
	boardCanvas.append("g")
				...
				.style("fill", function(d) {
					return (d.row + d.col) % 2 === 0 ? "#ada797" : "white";
				})
				.style("stroke", "black")
...
function drawText(data) {
	boardCanvas.append("g")
		...
		.style("fill", function(d) { return d.state === red ? "black" : "white"; })
...
function get_available_piece_moves(target_board, target_piece, player) {
    ...
    // Para Kings, permitir movimentos múltiplos
    if (Math.abs(from.state) === 1.1) {
        var directions = [-1, 1];
        directions.forEach(function(xMove) {
            directions.forEach(function(yMove) {
                var row = from.row + yMove;
                var col = from.col + xMove;
                while (row >= 0 && row < 8 && col >= 0 && col < 8) {
                    var cell_index = get_cell_index(target_board, col, row);
                    if (cell_index >= 0) {
                        var to = target_board.cells[cell_index];
                        if (isMoveLegal(target_board.cells, target_board.pieces, from, from, to)) {
                            moves.push({ move_type: 'slide', piece: player, from: { col: from.col, row: from.row }, to: { col: to.col, row: to.row } });
                        }
                        if (to.state != empty) {
                            var nextRow = row + yMove;
                            var nextCol = col + xMove;
                            if (nextRow >= 0 && nextRow < 8 && nextCol >= 0 && nextCol < 8) {
                                var nextCellIndex = get_cell_index(target_board, nextCol, nextRow);
                                if (nextCellIndex >= 0) {
                                    var nextTo = target_board.cells[nextCellIndex];
                                    if (isMoveLegal(target_board.cells, target_board.pieces, from, from, nextTo)) {
                                        moves.push({ move_type: 'jump', piece: player, from: { col: from.col, row: from.row }, to: { col: nextTo.col, row: nextTo.row } });
                                    }
                                }
                            }
                            break;
